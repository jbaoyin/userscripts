...
(function () {
  ...
  const Core = {
    ...
    cleanNode(node, attrs) {
      ...
      ["user-select", "-webkit-user-select"].forEach((p) =>
  ...
  const SITES = {
    "book.qq.com": {
      ...
      onReady(doc) {
        const reClean = () =>
          doc
            .querySelectorAll(".chapter-content,.read-content,.text-content")
            .forEach((el) => {
              el.style.removeProperty("user-select");
              el.style.removeProperty("-webkit-user-select");
              ["unselectable", "data-copyblock", "data-noselect"].forEach((a) =>
                el.removeAttribute(a),
              );
            });
        const t = doc.querySelector("#app") || doc.body;
        if (t)
          new MutationObserver(reClean).observe(t, {
            childList: true,
            subtree: true,
          });
        reClean();
